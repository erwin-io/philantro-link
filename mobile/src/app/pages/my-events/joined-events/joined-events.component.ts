...
export class JoinedEventsComponent implements OnInit {
  ...
  ngOnInit() {
    if(this.isAuthenticated) {
      this.pageIndex = 0;
      this.pageSize = 10;
      this.initJoinedEvents(true, true);
    }
  }
  ...
  async initJoinedEvents(showProgress = false, clear = false) {
    try {
      if(clear) {
        this.events = [];
      }
      this.isLoading = showProgress;
      this.supportTicketService.getPageJoinedEvents({
  ...
  async doRefresh(){
    try {
      ...
      this.pageIndex = 0;
      this.pageSize = 10;
      await this.initJoinedEvents(true, true);
    }catch(ex) {
      this.isLoading = false;
      if(this.ionRefresher) {
        this.ionRefresher.complete();
