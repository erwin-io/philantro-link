...
export class SupportTicketService implements IServices {
  ...
  getByCode(supportTicketCode: string, currentUserCode = ''): Observable<ApiResponse<SupportTicket>> {
    return this.http.get<any>(environment.apiBaseUrl + this.appconfig.config.apiEndPoints.supportTicket.getByCode + supportTicketCode + '?currentUserCode=' + currentUserCode)
    .pipe(
      tap(_ => this.log('support-ticket')),
      catchError(this.handleError('support-ticket', []))
    );
